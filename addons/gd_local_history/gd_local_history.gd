...
func _disable_plugin() -> void:
	...
	ProjectSettings.set_setting(_setting_allow_global_directory, null)
	ProjectSettings.set_setting(_setting_save_file_path, null)

func _enter_tree() -> void:
	# Init global directory project setting
	if (!ProjectSettings.has_setting(_setting_allow_global_directory)):
		ProjectSettings.set_setting(_setting_allow_global_directory, false)
	ProjectSettings.set_initial_value(_setting_allow_global_directory, false)
	ProjectSettings.set_restart_if_changed(_setting_allow_global_directory, true)
	var _global_directory_property_info: Dictionary = {
		"name": _setting_allow_global_directory,
		"type": TYPE_BOOL
	}
	ProjectSettings.add_property_info(_global_directory_property_info)

	# Init save file path project setting
	if (!ProjectSettings.has_setting(_setting_save_file_path)):
		ProjectSettings.set_setting(_setting_save_file_path, "res://.gd_local_history")
	ProjectSettings.set_initial_value(_setting_save_file_path, "res://.gd_local_history")
	ProjectSettings.set_as_basic(_setting_save_file_path, true)
	ProjectSettings.set_restart_if_changed(_setting_save_file_path, true)

	var _file_property_info: Dictionary = {}
	if (ProjectSettings.get_setting(_setting_allow_global_directory)):
		_file_property_info = {
			"name": _setting_save_file_path,
			"type": TYPE_STRING,
	...
	# Init save file path, and create file path if one does not exist
	save_file_path = ProjectSettings.get_setting(_setting_save_file_path)
